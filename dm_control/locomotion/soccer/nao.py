...
from dm_control.locomotion.walkers import nao
import numpy as np
import pdb
...
class NAO(nao.NAOPositionControlled):
  ...
  def _build(self,  # pytype: disable=signature-mismatch  # overriding-parameter-count-checks
    ...
    if not isinstance(visual, NAO.Visual):
    ...
    if walker_id is None and visual != NAO.Visual.GEOM:
      raise ValueError(
